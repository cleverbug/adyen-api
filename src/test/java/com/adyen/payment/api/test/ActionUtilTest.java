...
import static com.adyen.payment.api.model.ShopperInteraction.Ecommerce;
...
public class ActionUtilTest {
    ...
    public void testCreatePost() throws Exception {
        ...
        assertNotNull(request);
        PaymentRequest pr = PaymentRequestBuilder.merchantAccount("testAccount").amount(new Amount(Currency.getInstance("USD"), 10000L)).card(CardBuilder.number("4111111111111111").cvc("123").expiry(2016, 6).holder("Johnny Tester").build()).reference("ref001").additionalDataEntry("returnUrl", "http://www.adyen.com").shopper(NameBuilder.first("Johnny").last("Tester").build(), "willian.oki@gmail.com", "127.0.0.1", "Test/DAPI/Authorisation/PayULatam", Ecommerce).build();
