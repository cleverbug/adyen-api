...
public class Capture {
   ...
   private static Request createRequest(final ClientConfig config, final CaptureRequest request) {
      Request retval = null;
      String url;
      // create a Post
      try {
         url = config.getServices().get(APService.CAPTURE).toString();
      } catch(Exception e) {
         throw new APSConfigurationException("capture: missing parameter: url");
      }
   ...
   public static CaptureResponse execute(final ClientConfig config, final CaptureRequest request) {
      CaptureResponse retval = null;
      // create the request
      ...
      try {
         retval = exec.execute(req).handleResponse(
            new ResponseHandler<CaptureResponse>() {
               public CaptureResponse handleResponse(HttpResponse response) throws ClientProtocolException, IOException {
                  CaptureResponse retval = new CaptureResponse();
                  ...
                  if(entity == null) {
                     throw new ClientProtocolException("blank: capture response");
                  }
                  retval = MAPPER.fromJson(new InputStreamReader(entity.getContent()), CaptureResponse.class);
                  return retval;
               }
            }
         );
      } catch(Exception e) {
         throw new APSAccessException("capture", e);
      }
      return retval;
   }
}
